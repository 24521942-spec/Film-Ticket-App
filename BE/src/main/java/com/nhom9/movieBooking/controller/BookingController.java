...
public class BookingController {
    ...
    @GetMapping("/my")
    public List<BookingBriefDto> myBookings() {
        return bookingService.getMyBookings();
    }

    // @GetMapping("/{bookingId}")
    // public BookingDetailDto getDetail(@PathVariable Integer bookingId) {
    //     return bookingService.getBookingDetail(bookingId);
    // }

    @PostMapping("/fake")
    public ResponseEntity<?> createFake(@RequestBody FakeBookingRequestDto req) {
        return ResponseEntity.ok(bookingService.createFakeBooking(req));
    }
    @GetMapping("/{bookingId}")
    public BookingDetailDto getDetail(@PathVariable Integer bookingId) {
        return bookingService.getBookingDetailFull(bookingId);
}
